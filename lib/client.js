...
class BaseClient {
   ...
   * @api private
   */
  async dpopProof(payload, privateKey, accessToken) {
    if (!isPlainObject(payload)) {
      throw new TypeError('payload must be a plain object');
    }

    // TODO: when passing CryptoKeyPair / generateKeyPairResult this fails with a weird message.
    ...
    let alg = determineDPoPAlgorithm.call(this, privateKey, privateKey);
    ...
    return new jose.SignJWT({
      ...
      .setProtectedHeader({
        alg,
        typ: 'dpop+jwt',
        jwk: await this.getJwk(privateKey, privateKey),
      })
      .setIssuedAt()
      .setJti(random())
      .sign(privateKey);

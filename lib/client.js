...
class BaseClient {
  ...
  async callback(
    ...
    if (params.id_token) {
      ...
      await decryptIdToken.call(this, tokenset);
  ...
  async #validateJWTUserinfo(body) {
    ...
    return validateJWT.call(this, body, expectedAlg, [], {
      aadIssValidation: this.#aadIssValidation,
      additionalAuthorizedParties: this.#additionalAuthorizedParties,
    });
  }
  ...
  async #decryptJARM(response) {
    ...
    return decryptJWE.call(this, response, expectedAlg, expectedEnc);
  }
  ...
  async #decryptJWTUserinfo(body) {
    ...
    return decryptJWE.call(this, body, expectedAlg, expectedEnc);
  }
  ...
  async validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
    ...
    const {
      ...
      payload,
      key,
    } = await validateJWT.call(this, idToken, expectedAlg, undefined, {
      aadIssValidation: this.#aadIssValidation,
      additionalAuthorizedParties: this.#additionalAuthorizedParties,
    });
    ...
    if (returnedBy === 'authorization') {
      ...
      if (payload.s_hash) {
        ...
        try {
          ...
          throw new RPError({ message: 'failed to validate JWT s_hash', jwt: idToken, cause });
        }
      }
    }

    if (this.#fapi1() && payload.iat < timestamp - 3600) {
      throw new RPError({
    ...
    if (tokenSet.access_token && payload.at_hash !== undefined) {
      try {
        ...
        throw new RPError({ message: 'failed to validate JWT at_hash', jwt: idToken, cause });
      }
    }
    ...
    if (tokenSet.code && payload.c_hash !== undefined) {
      try {
        ...
        throw new RPError({ message: 'failed to validate JWT c_hash', jwt: idToken, cause });
      }
    }
  ...
  async requestResource(
    ...
    if (this.dpop_bound_access_tokens && !DPoP) {
      throw new TypeError('DPoP key not set');
    }
  ...
  async userinfo(accessToken, { method = 'GET', via = 'header', tokenType, params, DPoP } = {}) {
    ...
    if (jwt) {
      ...
      try {
        ...
        throw new RPError({
          message: 'failed to parse userinfo JSON',
          response,
          cause,
        });
      }
  ...
  async requestObject(
    ...
    if (signingAlgorithm === 'none') {
      signed = [
        ...
        jose.base64url.encode(payload),
        '',
      ].join('.');
    } else {
      const symmetric = signingAlgorithm.startsWith('HS');
      if (symmetric) {
        key = secretForAlg.call(this, signingAlgorithm);
      } else {
    ...
    if (fields.alg.match(/^(RSA|ECDH)/)) {
      ...
      key = secretForAlg.call(this, fields.alg === 'dir' ? fields.enc : fields.alg);
    }
  ...
  async pushedAuthorizationRequest(params = {}, { clientAssertionPayload, DPoP } = {}) {
    ...
    const response = await authenticatedPost.call(
      ...
      { clientAssertionPayload, endpointAuthMethod: 'token', DPoP },
    );
  ...
  async #validateJARM(response) {
    ...
    const { payload } = await validateJWT.call(this, response, expectedAlg, ['iss', 'exp', 'aud'], {
      aadIssValidation: this.#aadIssValidation,
      additionalAuthorizedParties: this.#additionalAuthorizedParties,
    });
